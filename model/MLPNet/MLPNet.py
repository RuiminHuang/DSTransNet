...
#from model_net.t2t_vit import Token_MLP
#from model_net.t2t_vit import Cannel_MLP
...
#
#         feat_ir = ir_temp.cpu().clamp(0, 30).data.numpy()*50
#         feat_vi = vi_temp.cpu().clamp(0, 30).data.numpy()*50
#
...
class TokenMixing(nn.Module):
    ...
    def __init__(self, C, H, W, alpha, use_dropout=False, drop_rate=0):
        super().__init__()
        self.smlp_block = PTIM(C, H, W, alpha, use_dropout=False, drop_rate=0)


    def forward(self, x):

        x = self.smlp_block(x) + x
...
class MLPNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = self.decoder(x1, x2, x3, x4, x_0)
...
if __name__ == '__main__':
    ...
    input_shape = (1, 3, 256, 256)  # 通常batch size是第一个维度
